...
function preguntarNumero() {
    return new Promise((resolve) => {
        ...
        rl.question('📱 Ingresa tu número de WhatsApp (con código de país, sin + ni espacios)\n   Ejemplo Colombia: 573001234567\n👉 Número: ', (respuesta) => {
            rl.close();
...
async function conectar() {
    ...
    if (!sock.authState.creds.registered) {
        let numero = OWNER_NUMBER;

        // Si el número aún no fue configurado, preguntar en consola
        if (numero === '57XXXXXXXXXX') {
        ...
        console.log(`\n✅ Tu código de emparejamiento es: \x1b[1m\x1b[32m${codigo}\x1b[0m`);
        console.log('📲 Ve a WhatsApp → Dispositivos vinculados → Vincular con número de teléfono');
        console.log('   Ingresa ese código y en segundos el bot estará activo.\n');
    }
    ...
    sock.ev.on('messages.upsert', async ({ messages }) => {
        ...
        // Usar el número configurado arriba, o el que se ingresó en consola
        const ownerActual = OWNER_NUMBER === '57XXXXXXXXXX'
            ? sock.authState.creds.me?.id?.split(':')[0]
            : OWNER_NUMBER;

        // Solo responde al dueño
        if (senderNumber !== ownerActual) {
